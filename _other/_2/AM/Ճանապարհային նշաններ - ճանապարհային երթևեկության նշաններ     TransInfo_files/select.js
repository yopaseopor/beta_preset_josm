	...
	choose: function() {
		...
		for(d = 0; d < option.length; d++) {
			if(option[d].selected == true) {
				...
				if(this.name=='region' && option[d].value==2)
				{
					 SetDisplayCity('none')	
				}
				else if( this.name=='region' &&  option[d].value!=2)
				{
					SetDisplayCity('block')		
				}
				//xmlhttpPost('ajax.php','select_m','en');
				if(this.name == 7) {
					var id = $("styled").value;
					var link = 'ajax.php?id='+id;
					...
					new Request.JSON({url: link, method: 'get', onSuccess: function(data) {
						var opt = $('sel2');
						opt.empty();
...
/*function getquerystring(idx, p)
{
	if(idx=="select_m")
	{
		//document.getElementById('ann_city_id').innerHTML = "<center><img src='/img/loading.gif' alt='' /></center>";
		m_id = document.getElementById("styled").value;
		qstr = 'm_id=' + escape(m_id) + '&lang_code=' + p;
	}
